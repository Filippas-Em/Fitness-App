...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($_POST['news_id']) || empty($_POST['announcement'])) {
        $response = [
            "success" => false,
    ...
    $news_id = $_POST['news_id'];  // Use the correct variable name
    $announcement = $_POST['announcement'];  // Use the correct variable name

    // Log the received variables for debugging
    error_log("Received variables: news_id = $news_id, announcement = $announcement");

    try {
        // Update query for the announcements table, specifically the info column
        $stmt = $pdo->prepare("UPDATE announcements 
            SET info = :announcement
            WHERE id = :news_id");
        ...
        if ($stmt->execute()) {
            ...
            echo json_encode(["success" => false, "message" => "Failed to update announcement.", "received_data" => $_POST]); // Include received data on failure
        }
    } catch (PDOException $e) {
