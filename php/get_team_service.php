...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id'])) {
    ...
    try {
        $stmt = $pdo->prepare("SELECT * FROM team_services WHERE id = :id");
        $stmt->bindParam(':id', $service_id, PDO::PARAM_INT);
        $stmt->execute();
        $teamService = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($teamService) {
            echo json_encode($teamService);
        } else {
            echo json_encode(["error" => "Team service not found."]);
        }
    } catch (PDOException $e) {
